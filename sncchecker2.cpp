...
//vector<char> place_into_something(fstream& file_handle, string path, vector<char>& vec, int& size){
//	int position = 0;
...
//		
//		while (!file_handle.eof() && position < size - 1)
//		{
//			file_handle.get(vec[position]);
//			position++;
//		}
//		file_handle.close();
//
...
int validate_devID(string file = " "){
	...
	if (file != " ") {
		...
		{ 
			...
			{
				fp.get(doom2[secposition]); 
				secposition++;
			}
			doom2[secposition - 1] = '\0'; 
	...
	//place_into_something(fp, path, doom, array_size);
...
bool direct_check_indirect(string pathname, int& notdirect, vector<char>& location){
	...
	fstream directive;
	directive.open(pathname);
	if (directive){
		while (!directive.eof() && position < array_size - 1)
		{
			directive.get(invective[position]); 
			position++;
		}
...
bool sizecheck(string pathname, int& indirection, vector<char>& current_spot){
	...
	plug.open(pathname);
	if (plug){
		while (!plug.eof() && position < array_size - 1)
		{
			plug.get(pocket[position]); 
			position++;
		}
		...
		for (int i = 0; i < pocket.size(); ++i){
			if (pocket[i] == 'l' && pocket[i + 1] == 'o' && pocket[i + 2] == 'c' && pocket[i + 3] == 'a' && pocket[i + 4] == 't'
				...
				if (pocket[i + 10] != ','){
					...
					//length_of_location_array = atoi(buffer);
	...
	int secposition = 0;
	if (outlet){
		while (!outlet.eof() && secposition < array_size - 1)
		{
			outlet.get(locations[secposition]); 
			secposition++;
		}
		outlet.close();
...
bool free_blockparty(string file, int& block_number, vector<int> &fblock_list, map<string, string> &freeblocks){
	...
	if (filer){
		...
		for (int i = 1; i < 26; ++i){
			...
			while (!file.eof() && position < array_size - 1)
			{
				file.get(block_party[position]); 
				position++;
			}
			file.close();
