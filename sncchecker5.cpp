...
//Idea for following function adapted from following website
...
bool days_of_future_past(string path = " "){
	...
	if (path == " "){
				...
				if (exo2[location2] == 'c' && exo2[location2 + 1] == 'r' && exo2[location2 + 2] == 'e' && exo2[location2 + 3] == 'a'
					...
					if (exo2[location2 + 13] != ' '){
						...
						if (time_value < ctime_value) {
							std::cout << "For fusedata.0, ctime value is in the past. True" << endl;
							//return true;
						}
		...
		if (path == " "){
			path = "fusedata.27";
			cout << "fusedata.27 :" << endl;
			days_of_future_past(path);
		}
...
void dot_block_checker(string file, int& current_number){
					...
					if (axel[location3 + 2] == '.'){
						if (axel[location3 + 3] == ':' || axel[location3 + 3] == '.'){
							if (axel[location3 + 3] == ':'){
								...
								char buffer[10];
								strcat_s(buffer, &axel[location3 + 4]);
								if (atoi(&axel[location3 + 5])){
									strcat_s(buffer, &axel[location3 + 5]);
								}
								if (atoi(buffer) == current_number){
									continue;
								}
								std::cout << "Incorrect directory listing" << endl;
								exit(1);
							}
							...
							if (axel[location3 + 4] == ':'){
								...
								char buffer[10];
								strcat_s(buffer, &axel[location3 + 5]);
								if (atoi(&axel[location3 + 6])){
									strcat_s(buffer, &axel[location3 + 6]);
								}
								if (atoi(buffer) == current_number){
									continue;
								}
...
bool sizecheck(string pathname, int& indirection, vector<char>& current_spot){
	...
	if (indirection == 0 && size <= 4096 && size != 0){
...
bool free_blockparty(string filez, int& block_number, vector<int> &fblock_list, map<string, string> &freeblocks){
	...
	if (filer){
		...
		std::cout << "There is a free block that wasn't found in the free block list." << endl;
		std::cout << "Here is the block number for reference: " + block_number << endl; 
		return false;
	}
	else {
		std::cout << "File could not be opened " << endl;
...
bool free_block_list_check(map<string, string> &FreeBlocks){
	for each(pair<string, string> blocks in FreeBlocks){
		...
		if (!file_read.empty()){
			std::cout << "There exists a file belonging to the free block list that is currently not empty" << endl;
			std::cout << "Here is the block number for reference: " + blocks.first << endl;
			return false;
		}
	}
	return true;
...
bool csefsck(){
	...
	for (int i = 0; i < 10000; ++i){
		fstream file("fusedata/fusedata." + i);
		if (file.is_open()){
			stringstream stream;
			stream << file.rdbuf();
			...
			file.close();
			if (result_dev != 0) {
				std::cout << "Dev ID has reported that this is not your filesystem. I cannot allow you to go any further" << endl;
				//exit(1);
			}
			bool time_stone = days_of_future_past();
			if (!time_stone){
				std::cout << "Looks like one of the times listed in a block is not in the past" << endl;
				//exit(1);
			}
			bool link_test = link_matcher();
			if (!link_test){
				std::cout << "Number of links is inconsistent with multiple blocks" << endl;
				//exit(1);
				return false;
			}
			dot_block_checker(filer, i);
			bool indirect_check = direct_check_indirect(readFile, indirect, locations);
			if (!indirect_check){
				std::cout << "Indirect has not been correctly set" << endl;
				//exit(1);
				return false;
			}
			...
			if (!size){
				std::cout << "It was reported that the size for specificed parameters is invalid" << endl;
				//exit(1);
				return false;
			}
			...
			if (!blockade){
				std::cout << "There is something not contained in the free block list that should be" << endl;
				//exit(1);
				return false;
			}
	...
	if (!blockparty){
		std::cout << "There is something in the free block list that shouldn't be there" << endl;
		return false;
	}
	return true;
}
